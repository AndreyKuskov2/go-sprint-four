...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speedInMInS := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMInS, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
